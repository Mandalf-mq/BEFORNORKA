...
const AccountCSVImporter: React.FC<AccountCSVImporterProps> = ({ onSuccess, onClose }) => {
  ...
  const validateAccountData = (data: any[]): string[] => {
    const errors: string[] = [];
    ...
    if (selectedFile && selectedFile.type === 'text/csv') {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Validation
          const errors = validateAccountData(parsedData);
          setValidationErrors(errors);
        } catch (error: any) {
  ...
  const handleImport = async () => {
    ...
    try {
      // Pour l'instant, simuler l'import (la vraie fonctionnalité nécessite une Edge Function)
