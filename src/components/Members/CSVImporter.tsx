...
🔄 Les nouveaux menus apparaîtront à sa prochaine connexion`);
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'users' && (
        <div className="bg-white rounded-xl p-6 shadow-lg">
          p_csv_data: csvDataWithPasswords
        });

        if (error) throw error;
        
        // Enrichir le résultat avec les mots de passe suggérés
        const enrichedResult = {
          ...data,
          credentials: csvDataWithPasswords.map(row => ({
            email: row.email,
            name: `${row.first_name} ${row.last_name}`,
            password: row.suggested_password
          }))
        };
        
        setImportResult(enrichedResult);

        if (data.success) {
          alert(`✅ Import terminé (mode fallback) !

📊 Résultats :
• ${data.imported_count} profils créés
• ${data.error_count} erreurs
• Identifiants suggérés générés

📋 INSTRUCTIONS :
• Communiquez les identifiants aux utilisateurs
• Ils doivent s'inscrire manuellement sur /auth
• Leurs profils seront automatiquement liés`);
        }
          <h3 className="text-lg font-semibold text-gray-900 mb-6">
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <p className="text-xs text-yellow-700 mb-2">
                          ⚠️ Mode fallback : Les utilisateurs doivent s'inscrire manuellement avec ces identifiants
                        </p>
                          <option value="webmaster">👑 Webmaster</option>
                        )}
                      </select>
                              <strong>{cred.name}</strong> - {cred.email} - Mot de passe suggéré : {cred.password}
