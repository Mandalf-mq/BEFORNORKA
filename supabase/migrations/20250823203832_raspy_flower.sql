...
-- Fonction pour traiter les liens familiaux après import
CREATE OR REPLACE FUNCTION process_family_links_from_csv(
  p_csv_data jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  ...
  parent_id uuid;
BEGIN
  -- Parcourir les données pour traiter les liens familiaux
  FOR member_data IN SELECT * FROM jsonb_array_elements(p_csv_data)
  LOOP
    BEGIN
      -- Si family_head_email est renseigné, créer le lien
      IF (member_data->>'family_head_email') IS NOT NULL AND (member_data->>'family_head_email') != '' THEN
        ...
        FROM members 
        WHERE email = (member_data->>'family_head_email');
        
        IF child_id IS NOT NULL AND parent_id IS NOT NULL THEN
          -- Créer le lien familial
          UPDATE members 
          SET 
            family_head_id = parent_id,
        ...
        ELSE
          errors := array_append(errors, (member_data->>'email') || ': Parent ou enfant non trouvé pour lien familial');
          error_count := error_count + 1;
        END IF;
      END IF;
      
    EXCEPTION WHEN OTHERS THEN
      errors := array_append(errors, (member_data->>'email') || ': Erreur lien familial - ' || SQLERRM);
      error_count := error_count + 1;
    END;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'error_count', error_count,
    'errors', errors,
    'message', processed_count || ' liens familiaux créés'
  );
END;
$$;