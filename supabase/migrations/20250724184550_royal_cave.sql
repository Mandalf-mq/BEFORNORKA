/*
  # Nettoyage sécurisé et restructuration de la base de données

  1. Nettoyage sécurisé
    - Suppression des fichiers existants dans les buckets
    - Suppression sécurisée des buckets
    - Nettoyage des politiques RLS

  2. Restructuration propre
    - Création des nouvelles tables avec structure cohérente
    - Buckets Storage avec noms définitifs
    - Politiques RLS simples et efficaces

  3. Types de documents finaux
    - ffvbForm, medicalCertificate, idPhoto, parentalConsent
    - Contraintes CHECK correctes
    - Cohérence totale frontend/backend
*/

-- ========================================
-- ÉTAPE 1: NETTOYAGE SÉCURISÉ DES BUCKETS
-- ========================================

-- Supprimer tous les fichiers des anciens buckets
DELETE FROM storage.objects WHERE bucket_id IN ('documents', 'templates', 'member-documents', 'document-templates');

-- Supprimer les buckets existants (maintenant vides)
