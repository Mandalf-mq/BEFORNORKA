...
-- ÉTAPE 3: CORRECTION DIRECTE (UPDATE SIMPLE)
UPDATE users 
...
-- ÉTAPE 5: SI ÉCHEC, DÉSACTIVER LE TRIGGER ET RÉESSAYER
DO $$
DECLARE
  current_role text;
BEGIN
...
-- ÉTAPE 7: MISE À JOUR DES MÉTADONNÉES AUTH POUR COHÉRENCE
UPDATE auth.users 
SET raw_user_meta_data = COALESCE(raw_user_meta_data, '{}'::jsonb) || '{"role": "webmaster"}'::jsonb
WHERE email = 'de.sousa.barros.alfredo@gmail.com';

-- ÉTAPE 8: VÉRIFICATION COMPLÈTE FINALE
SELECT 
