/*
  # CORRECTION DÉFINITIVE DES ERREURS DE FONCTIONS SQL
  
  1. Suppression explicite de toutes les fonctions existantes
  2. Recréation avec les bons types de retour
  3. Implémentation complète du workflow
  4. Système de rôles et permissions
  5. Création de comptes membres avec mot de passe temporaire
  6. Import CSV pour ajout en lot
  7. Gestion complète des saisons
  8. Correction des uploads de documents
*/

-- ========================================
-- ÉTAPE 1: SUPPRESSION EXPLICITE DE TOUTES LES FONCTIONS EXISTANTES
-- ========================================

-- Supprimer toutes les versions de validate_member_profile
DROP FUNCTION IF EXISTS validate_member_profile(uuid, text);
DROP FUNCTION IF EXISTS validate_member_profile(uuid);

-- Supprimer toutes les versions de validate_document
DROP FUNCTION IF EXISTS validate_document(uuid, text, text);
DROP FUNCTION IF EXISTS validate_document(uuid, text);

-- Supprimer toutes les versions de create_new_season_and_reset_members
...
-- Supprimer toutes les versions de create_member_account_with_password
DROP FUNCTION IF EXISTS create_member_account_with_password(text, text, text, text, date, text, text, integer);
DROP FUNCTION IF EXISTS create_member_account_with_password(text, text, text, text, date, text, text);

-- Supprimer toutes les versions de import_members_from_csv
DROP FUNCTION IF EXISTS import_members_from_csv(jsonb);

-- Supprimer toutes les versions de delete_category
DROP FUNCTION IF EXISTS delete_category(uuid);

-- Supprimer toutes les versions de get_required_documents_for_member
DROP FUNCTION IF EXISTS get_required_documents_for_member(uuid);

-- Supprimer toutes les versions de check_all_required_documents_validated
DROP FUNCTION IF EXISTS check_all_required_documents_validated(uuid, uuid);

-- Supprimer toutes les versions de get_membership_fee_by_category
DROP FUNCTION IF EXISTS get_membership_fee_by_category(text);

-- Supprimer toutes les versions de get_category_by_age
DROP FUNCTION IF EXISTS get_category_by_age(date);

-- Supprimer toutes les versions de calculate_age
...
-- ========================================
-- ÉTAPE 5: RECRÉER TOUTES LES FONCTIONS UTILITAIRES
-- ========================================
...
CREATE FUNCTION get_category_by_age(birth_date date)
RETURNS text AS $$
DECLARE
  member_age integer;
BEGIN
...
CREATE FUNCTION get_membership_fee_by_category(category_value text)
RETURNS integer AS $$
DECLARE
  fee integer;
BEGIN
...
-- ========================================
-- ÉTAPE 12: TRIGGERS POUR TRANSITIONS AUTOMATIQUES
-- ========================================
...
-- ========================================
-- ÉTAPE 14: FONCTION DE DIAGNOSTIC DU WORKFLOW
-- ========================================
...
DO $$
BEGIN
  RAISE NOTICE '🎉 TOUTES LES FONCTIONS SQL CORRIGÉES !';
  RAISE NOTICE '';
  RAISE NOTICE '✅ CORRECTIONS APPLIQUÉES :';
  RAISE NOTICE '  - Toutes les fonctions supprimées explicitement puis recréées';
  RAISE NOTICE '  - Types de retour corrigés (jsonb pour toutes)';
  ...
  RAISE NOTICE '  - create_member_account_with_password(email, prénom, nom, tél, naissance, catégorie, mdp)';
  RAISE NOTICE '  - validate_member_profile(id, notes) → jsonb';
  RAISE NOTICE '  - validate_document(id, action, reason) → jsonb';
  RAISE NOTICE '  - create_new_season_and_reset_members(...) → jsonb';
  RAISE NOTICE '  - import_members_from_csv(data) → jsonb';
  RAISE NOTICE '  - delete_category(id) → jsonb';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 WORKFLOW COMPLET :';
  RAISE NOTICE '  pending → validated → documents_pending → documents_validated → season_validated';
  RAISE NOTICE '';
  RAISE NOTICE '🚀 PRÊT POUR LES TESTS COMPLETS !';
END $$;